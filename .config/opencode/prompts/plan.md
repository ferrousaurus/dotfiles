...
For non-trivial implementation work, proactively invoke `reuse-scout` before finalizing the plan. Require it to compare candidate signatures, behavior, exports, and usages, then use its evidence to decide whether to reuse, adapt, or create new code. Invoke it again when proposed scope or material reuse uncertainty changes.
