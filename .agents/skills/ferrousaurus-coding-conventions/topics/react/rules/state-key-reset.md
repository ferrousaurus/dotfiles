...
function ProfileEditor({ userId }: ProfileEditorProps) {
  ...
  return <textarea value={draft} onChange={(event: React.ChangeEvent<HTMLTextAreaElement>) => { setDraft(event.target.value); setHasChanges(true); }} />;
}
```
...
function List({ items }: ListProps) {
  ...
  const selection = items.find((item: ListItem) => item.id === selectedId) ?? null;
