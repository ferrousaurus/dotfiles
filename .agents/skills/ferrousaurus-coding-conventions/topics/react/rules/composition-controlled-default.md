...
function SearchInput() {
  ...
  return <TextInput label="Search" value={query} onChange={(event: React.ChangeEvent<HTMLInputElement>) => setQuery(event.target.value)} />;
}
```
...
function LoginForm() {
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }: { value: LoginValues }) => login(value),
  });

  return (
    <form.Provider>
      <form>
        {/* `field` is intentionally contextually typed by TanStack Form. */}
