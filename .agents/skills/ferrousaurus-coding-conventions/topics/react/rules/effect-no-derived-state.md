...
function TodoList({ todos, filter }: TodoListProps) {
  ...
  return <ul>{visibleTodos.map((todo: Todo) => <li key={todo.id}>{todo.text}</li>)}</ul>;
}
```
...
```tsx
function TodoList({ todos, filter }: TodoListProps) {
  const visibleTodos = todos.filter((todo: Todo) => !todo.completed && todo.text.includes(filter));
  return <ul>{visibleTodos.map((todo: Todo) => <li key={todo.id}>{todo.text}</li>)}</ul>;
}
```
...
function TodoList({ todos, filter }: TodoListProps) {
  const visibleTodos = useMemo(
    () => todos.filter((todo: Todo) => !todo.completed && todo.text.includes(filter)),
    [todos, filter],
  );
...
- Related: [hook-memoize-sparingly.md](../../react/rules/hook-memoize-sparingly.md)
