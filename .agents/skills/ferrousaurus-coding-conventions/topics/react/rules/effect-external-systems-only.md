...
```tsx
function SearchResults({ query }: SearchResultsProps) {
  const filteredResults = results.filter((result: SearchResult) => result.name.includes(query));
  return <ul>{filteredResults.map(...)}</ul>;
}
```
...
function SearchResults({ query }: SearchResultsProps) {
  const filteredResults = useMemo(
    () => results.filter((result: SearchResult) => result.name.includes(query)),
    [results, query],
  );
  return <ul>{filteredResults.map(...)}</ul>;
}
...
- Related: [effect-no-derived-state.md](../../react/rules/effect-no-derived-state.md)
- Related: [data-tanstack-query-only.md](../../react/rules/data-tanstack-query-only.md)
- Related: [effect-sync-external-store.md](../../react/rules/effect-sync-external-store.md)
- Related: [state-key-reset.md](../../react/rules/state-key-reset.md)
