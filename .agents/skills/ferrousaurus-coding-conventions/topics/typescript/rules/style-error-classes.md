...
Custom errors enable type-safe error handling after narrowing:
...
Catch bindings are unannotated because thrown values can be anything. Narrow with `instanceof Error` before using Error properties; do not use the invalid `catch (error: Error)` syntax.
